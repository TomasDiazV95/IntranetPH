...
function GestionCompromiso() {
  ...
  const descargarExcel = async () => {
    ...
    if (fechaInicio !== fechaFin) {
      alert('Solo puedes consultar un único día. Selecciona el mismo valor para ambas fechas.');
      return;
    }
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'GestionCompromiso');
  
      const nombre = nombreCarteras[cartera] || `Cartera_${cartera}`;
      const blob = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
      saveAs(new Blob([blob]), `GestionCompromiso_${nombre}_${fechaInicio}.xlsx`);
    } catch (err) {
  ...
  return (
    <Container className="mt-4">
      <h3>Descargar Gestión con Fecha de Compromiso</h3>
      <Form>
...
export default GestionCompromiso;
